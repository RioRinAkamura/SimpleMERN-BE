...
const agon2 = require("argon2")  //Hash password
...
router.post("/register", async (req, res) => {
    ...
    if (!username || !password) {
        res.status(400).json({ success: false, message: "Missing username and/of password" })
    }
    ...
    try {
        ...
        if (user) { res.status(400).json({ success: false, message: "Username already exist" }) }

        // All good
        const hashsedPassword = await agon2.hash(password)
...
router.post("/login", async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ success: true, message: "Logged in succesfully!", accessToken })
    } catch (error) {
        console.log(error);
        res.status(500).json({ success: false, message: "Internal server error" })
    }
